...
#[inline(always)]
fn is_valid_nonce_hash(data_hash: &[u8], nonce: u64, target_number_of_hashes: u64) -> bool {
...
fn first_bytes_as_u64(hash: Vec<u8>) -> u64 {
    BigUint::from_bytes_le(&hash[..NUMERATOR_BYTE_LENGTH])
        .to_u64()
        .unwrap()
}
...
fn hash_with_nonce(nonce: u64, data: &[u8]) -> Vec<u8> {
    ...
    sha256([data.to_vec(), nonce_big_uint.to_bytes_le()].concat())
}
