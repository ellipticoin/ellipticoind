...
const SENDER = new Uint8Array([...Buffer.from("0000000000000000000000000000000000000001", "hex")]);
const RECEIVER = new Uint8Array([...Buffer.from("0000000000000000000000000000000000000002", "hex")]);
...
describe('BaseToken', function() {
  ...
  beforeEach(async () => {
    ...
    await token.loadFile("../target/wasm32-unknown-unknown/debug/token.wasm");
  });
  ...
  describe('transfer', function() {
    it('decreases the senders balance by the amount specified', async function() {
      token.setBalance(SENDER, 100)
      let result = await token.call('transfer', RECEIVER, 20);
      assert.equal(token.getBalance(SENDER), 80);
    });

    it('increases the receivers balance by the amount specified', async function() {
      token.setBalance(SENDER, 100)
      let result = await token.call('transfer', RECEIVER, 20);
      assert.equal(token.getBalance(RECEIVER), 20);
    });
