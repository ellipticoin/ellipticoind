...
impl Contract for Exchange {
    const NAME: contract::Name = contract::Name::Exchange;
}

db_accessors!(Exchange {
...
mod tests {
    ...
    fn test_create_pool() {
        ...
        Exchange::create_pool(&mut db, ALICE, 1, APPLES, BASE_FACTOR).unwrap();

        assert_eq!(
            Token::get_balance(&mut db, ALICE, Exchange::liquidity_token(APPLES)),
            1
        );
        assert_eq!(
            Exchange::get_liquidity_providers(&mut db, APPLES,)
                .iter()
                .cloned()
                .collect::<Vec<Address>>(),
            vec![ALICE]
    ...
    fn test_recreate_pool() {
        ...
        Exchange::create_pool(&mut db, ALICE, 1, APPLES, BASE_FACTOR).unwrap();
        assert_eq!(
            Exchange::create_pool(&mut db, ALICE, 1, APPLES, BASE_FACTOR)
                .err()
                .unwrap()
                .to_string(),
    ...
    fn test_create_pool_insufficient_token_balance() {
        ...
        assert_eq!(
            Exchange::create_pool(&mut db, ALICE, 2, APPLES, BASE_FACTOR / 2)
                .err()
                .unwrap()
                .to_string(),
            "Insufficient balance of a000000000000000000000000000000000000000"
        );
        db.revert();
    ...
    fn test_create_pool_insufficient_base_token_balance() {
        ...
        assert_eq!(
            Exchange::create_pool(&mut db, ALICE, 2, APPLES, BASE_FACTOR * 2)
                ...
                .unwrap()
                .to_string(),
            "Insufficient balance of 6b175474e89094c44da98b954eedeac495271d0f"
        );
        db.revert();

        assert_eq!(
            Token::get_balance(&mut db, Exchange::liquidity_token(APPLES), ALICE),
            0
        );
    ...
    fn test_add_liquidity() {
        ...
        Exchange::create_pool(&mut db, ALICE, 1, APPLES, BASE_FACTOR).unwrap();
        Exchange::add_liquidity(&mut db, ALICE, 1, APPLES).unwrap();
    ...
    fn test_add_to_existing_liquidity() {
        ...
        Exchange::create_pool(&mut db, ALICE, BASE_FACTOR, APPLES, BASE_FACTOR).unwrap();
        Exchange::add_liquidity(&mut db, ALICE, BASE_FACTOR, APPLES).unwrap();
        Exchange::add_liquidity(&mut db, ALICE, BASE_FACTOR, APPLES).unwrap();
    ...
    fn test_remove_liquidity() {
        ...
        Exchange::create_pool(&mut db, ALICE, BASE_FACTOR, APPLES, BASE_FACTOR).unwrap();
        Exchange::add_liquidity(&mut db, ALICE, BASE_FACTOR, APPLES).unwrap();
        Exchange::remove_liquidity(&mut db, ALICE, BASE_FACTOR / 2, APPLES).unwrap();
    ...
    fn test_trade() {
        ...
        Exchange::create_pool(&mut db, ALICE, 100 * BASE_FACTOR, APPLES, BASE_FACTOR).unwrap();
        Exchange::create_pool(&mut db, ALICE, 100 * BASE_FACTOR, BANANAS, BASE_FACTOR).unwrap();
        Exchange::trade(&mut db, BOB, 100 * BASE_FACTOR, BANANAS, 0, APPLES).unwrap();
    ...
    fn test_trade_with_invariant_overflow() {
        ...
        Exchange::trade(&mut db, BOB, 996_007, BASE_TOKEN.clone(), 0, APPLES.clone()).unwrap();
        assert_eq!(
            Token::get_balance(&mut db, BOB, APPLES.clone(),),
            99_999_401_499
        );
        ...
        Exchange::remove_liquidity(&mut db, ALICE, BASE_FACTOR, APPLES).unwrap();
